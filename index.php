            ...
            if ($dir !== '.' && $dir !== '' && (!$dirFull || !is_dir($dirFull))) {
                ...
                $content = isset($_POST['new_content']) ? (string)$_POST['new_content'] : '';
                        ...
                        if ($baseTitle === '') {
		                    ...
		                    if (!empty($MDW_PUBLISHER_MODE)) {
		                        $subtitleInput = trim((string)($draftMetadata['page_subtitle'] ?? ''));
		                        if ($subtitleInput !== '') {
		                            // page_subtitle metadata is rendered as the H2; do not duplicate it in the body.
		                            $content = '';
		                        } else {
		                            $subtitleFallback = function_exists('mdw_t')
		                                ? trim((string)mdw_t('newMdMeta_page_subtitle', 'Page subtitle'))
		                                : 'Page subtitle';
		                            if ($subtitleFallback === '') $subtitleFallback = 'Page subtitle';
		                            $content = "## " . $subtitleFallback . "\n";
		                        }
		                    } else {
		                        $content = "# " . $baseTitle . "\n";
		                    }
		                ...
		                if (!empty($MDW_PUBLISHER_MODE)) {
		                    ...
		                    // A new note can never start in a deletion state, even if
		                    // a client submits stale or hidden metadata.
		                    $opts['set']['publishstate'] = 'Concept';
...
<body class="app-body index-page <?=h($folderIconClass)?> <?= $paneHeaderOrder === 'title_left' ? 'pane-header-title-left' : '' ?> <?= $indexSplitLayout ? 'index-split-layout' : '' ?> <?= $hideMarkdownEditor ? 'hide-markdown-editor' : '' ?>">
                ...
                <div class="editor-pane-inner">
                    ...
                    <?php
                        explorer_view_render_tree([
                            ...
                            'server_superuser' => $MDW_SERVER_AUTH_ROLE === 'superuser',
...
<div class="preview-container">
    <?php if ($mode === 'view' && $requested): ?>
        <div class="preview-copy-toolbar" data-copy-buttons="1" <?= $hideMarkdownEditor ? 'data-auth-superuser="1" ' : '' ?><?= ($copyButtonsEnabled || ($github_pages_plugin_loaded && $github_pages_env_ready) || $requested) ? '' : 'hidden' ?>>
...
<?php
$settings_modal_editor_context = false;
...
?>
		...
		window.MDW_IMAGES_URL = <?= mdw_json_for_script($assetImagesPath) ?>;
		</script>
